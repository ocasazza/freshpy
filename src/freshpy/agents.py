...
def get_requesters(freshpy_object, verify_ssl=True):
    """This function retrieves all requesters in a project
    ...
    :returns: JSON data with the requester user data
    ...
    while page < page_limit:
        uri = f"requesters?page={page}"
        response = api.get_request_with_retries(
            freshpy_object, uri, verify_ssl=verify_ssl
        )
        if "requesters" not in response or not response["requesters"]:
            break
        requesters.extend(response["requesters"])
        page += 1
    return requesters
...
def get_user_info(freshpy_object, lookup_value, verify_ssl=True):
    ...
    elif isinstance(lookup_value, int) or (
        isinstance(lookup_value, str) and lookup_value.isdigit()
    ):
        uri = f"agents/{lookup_value}"
        agent_data = api.get_request_with_retries(
            freshpy_object, uri, verify_ssl=verify_ssl
        )
        agent_data = agent_data["agent"] if "agent" in agent_data else agent_data
    else:
        raise errors.exceptions.InvalidFieldError(
            "An invalid Agent ID or email address was provided."
        )
...
def get_all_agents(
    freshpy_object, only_active=None, only_inactive=None, verify_ssl=True
):
...
def get_agent_id(freshpy_object, email, verify_ssl=True):
    ...
    if "status_code" in agent_data and agent_data["status_code"] == 404:
        raise errors.exceptions.NotFoundResponseError(
            "An agent ID was not found for the provided email address."
        )
